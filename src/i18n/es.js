// noinspection HtmlUnknownTarget
/**
 LANG-ES
 Idioma español.

 @author Pablo Pizarro R. @ppizarror.com
 @license MIT
 */

lang_db.es = {
    "about_title": "Acerca de",
    "about_title_content": "Tarea 2 curso CC-7515 Computación en GPU, Universidad de Chile. Visualizador shaders conjunto de Julia.<br><br>Autor: <a href='{3}'>{2}</a><br><br>Versión {0} ({1})<br><i class='fab fa-github'></i> <a href='https://github.com/ppizarror/julia-shaders-threejs'>Ver en github</a>",
    "answer_cancel": "Cancelar",
    "answer_establish": "Establecer",
    "answer_no": "No",
    "answer_yes": "Sí",
    "app_dialog_unknown_size": "Se intenta crear un diálogo con un tamaño <{0}> desconocido",
    "building_full_loading_layer": "Iniciando capa de carga",
    "cookie_disabled": "Las cookies están desactivadas, utilizando localStorage",
    "dat_gui_close": "Cerrar GUI",
    "dat_gui_open": "Abrir GUI",
    "dialog_error_button_function_null": "Se intenta asociar un evento inválido a un botón ({0}) del diálogo, se ha definido una función nula por defecto",
    "dialog_form_cancel": "Cancelar",
    "dialog_form_send": "Enviar",
    "dialog_form_submit_null": "La función submit no puede ser nula",
    "dialog_last_closed_equal_opened": "Se ha cerrado el último diálogo ya que se ha abierto uno idéntico",
    "dialog_popup_created": "Abriendo diálogo <{0}>",
    "error_cookie_load": "Error crítico al cargar las cookies de sesión",
    "error_loading_shader": "Error al cargar shader",
    "error_loading_shader_info": "Ocurrió un error fatal al cargar el archivo del shader <i>{0}</i>.",
    "error_shader_compile": "Error al compilar el shader",
    "error_shader_compile_info": "Ocurrió un error fatal al compilar el shader, revise el output de la consola para más detalles",
    "julia_constant": "Constante julia",
    "loading_shader": "Cargando el shader <{0}>",
    "menu_shader": "Shader",
    "menu_title": "Julia-Shaders",
    "shader_complex_title": "Plano complejo",
    "shader_data_color": "Color del shader",
    "shader_data_length": "Largo",
    "shader_iters": "N° iteraciones max",
    "shader_zoom_count": "Nivel zoom",
    "viewer_3d_error_draw_null": "ShaderViewer: Se ha intentado dibujar un objeto nulo",
    "viewer_3d_error_zobj": "ShaderViewer: Ocurrió un error al obtener el parámetro z del objeto {0}",
    "viewer_3d_gui_camera": "Cámara",
    "viewer_hide_zoom": "Ocultando plano de zoom",
    "viewer_show_zoom": "Mostrando plano de zoom",
};
